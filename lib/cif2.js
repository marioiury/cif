(function(){
	...
	(function(){
		...
		var getFilterMap = function( filterName ){
			
			var filterMap = this.filterMap || {};
			
			if ( filterName !== null ){
			
				var params = this.filters[ filterName ].params;
				for ( var param in params ){
					if ( Object.prototype.hasOwnProperty.call( params, param ) && params[ param ].values.length > 0 ){
						filterMap[ param ] = params[ param ].values;
					}
				}
		...
		var filterRun = function( block, name, param, value ){
			...
			if ( typeof this.widgets[ block ] !== "undefined"
			  && System.inArray( param, this.widgets[ block ].params ) ){
				...
				var filterName = this.widgets[ block ].filter;
				console.log( System.cut( this.widgets[ block ].params, getFilterMap.call( this, filterName ) ) );
				return;
			}
			
			//TODO: Возможно придется перенести в цикл ниже чтобы фильтры с иерархии выше не затирались
			//if ( this.blocks[ block ].issetFilter )
			//	this.filterMap = getFilterMap.call( this, this.blocks[block].filter );
			var filterMap = getFilterMap.call( this, this.blocks[block].filter );
					...
					if ( System.inArray( param, this.widgets[ b ].params ) ){
						if ( this.widgets[ b ].issetFilter
						  ...
						  && this.filters[ this.widgets[b].filter ].params[ param ].values.length > 0 )
							...
							var filterName = this.widgets[ b ].filter;
							console.log( System.cut( this.widgets[ b ].params, getFilterMap.call( this, filterName ) ) );
						}
					}
				}
				//b - Block
